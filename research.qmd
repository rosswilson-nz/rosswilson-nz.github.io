...
```{r}
make_article_citation <- function(authors, title, journal, volume, issue, pages, year, doi,
                                  link = TRUE) {
    glue(
        "{authors}. {title}. *{journal}* {year};{volume}{issue}{pages}. doi:[{doi}](https://doi.org/{doi})<br><br>",
        title = if (link) glue("[{title}]({fs::path('articles', doi, 'index.qmd')})") else title,
...
create_article_page <- function(
    ...
    fs::dir_create("articles", doi)
    file <- fs::file_create("articles", doi, "index.qmd")
    ...
    contents <- c(
        ...
        glue("{date}"),
        "\nCITATION",
        glue("{make_article_citation(authors, title, journal, volume, issue, pages, year, doi, link = FALSE)}"),
        "\n### Abstract",
        glue("\n{abstract}")
    )
...
make_presentation_citation <- function(conference, location, date, title, note) {
  glue(
...
make_publication_citation <- function(title, with, note, date, available) {
  glue(
    ...
    with = if_else(is.na(with), "", glue("With {with}. ", .trim = FALSE, .na = NULL)),
    note = if_else(is.na(note), "", glue("{note} ", .trim = FALSE, .na = NULL)),
    available = if_else(
        is.na(available),
        "",
...
articles <- tribble(
  ~authors, ~title, ~journal, ~volume, ~issue, ~pages, ~year, ~doi, ~abstract, ~date,
  "Pryymachenko Y, Wilson R", "The long-term impacts of opioid use before and after joint arthroplasty: matched cohort analysis of New Zealand linked register data", "Family Practice", 41, "6", NA_character_, 2024, "10.1093/fampra/cmad112", NA_character_, "16 November 2023",
  "Reynolds A, Hood F", "Protocol for healthy heart study", "BMJ Open", 13, NA_character_, "e074278", 2023, "10.1136/bmjopen-2023-074278", NA_character_, "9 November 2023"
)

# Format citations
articles <- articles %>% 
    mutate(
        citation = make_article_citation(authors, title, journal, volume, issue, pages, year, doi),
        metrics = get_article_metrics(doi)
    ) %>% 
    rowwise() %>% 
    mutate(
        link = create_article_page(authors, title, journal, volume, issue, pages, year, doi,
                                   abstract, date)
    )

articles %>%
        select(citation, metrics) %>%
        tt(colnames = FALSE) %>% 
...
publications <- tribble(
  ~title, ~with, ~note, ~date, ~available,
  "Statistical Analysis Plan for Reducing the burden of knee osteoarthritis through community pharmacy: A randomised controlled trial of the Knee Care for Arthritis through Pharmacy Service", "James Stanley and Ben Darlow", NA_character_, "18 October 2024", "https://hdl.handle.net/10523/42913",
  "Economic evaluation of the effects of manual therapy for knee osteoarthritis: Study protocol", "Yana Pryymachenko, Haxby Abbott, and Cathy Chapple", NA_character_, "17 May 2024", "https://osf.io/arn9v/"
)
# Format citations
...
presentations <- tribble(
  ~conference, ~location, ~date, ~title, ~note,
...
presentations <- presentations %>% 
    mutate(
        presentation = make_presentation_citation(conference, location, date, title, note)
    )
